...
// Creating tests with categories
func (r *Repository) CreateTest(ctx context.Context, test *domain.Test) error {
	...
	err = tx.QueryRowxContext(ctx, query, test.Name).Scan(&test.ID)
	if err != nil {
		return fmt.Errorf("failed to create test: %w", err)
	}
	...
	for _, cat := range test.Categories {
		...
		_, err := tx.ExecContext(ctx, query, test.ID, cat.ID)
		if err != nil {
...
func (r *Repository) ReadTestByID(ctx context.Context, id int) (*domain.Test, error) {
	...
	if err := r.db.GetContext(ctx, test, query, id); err != nil {
	...
	cats, err := r.GetCategoriesByTestId(ctx, id)
	if err != nil {
		return nil, fmt.Errorf("failed to get test: %w", err)
	}
...
func (r *Repository) ReadTestByName(ctx context.Context, name string) (*domain.Test, error) {
	...
	if err := r.db.GetContext(ctx, test, query, name); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, fmt.Errorf("test not found")
		}
		return nil, fmt.Errorf("failed to get test: %w", err)
	...
	// Get categories for test
	cats, err := r.GetCategoriesByTestId(ctx, test.ID)
	if err != nil {
		return nil, fmt.Errorf("failed to get test: %w", err)
	}
...
func (r *Repository) UpdateTest(ctx context.Context, test *domain.Test) error {
	...
	// testInDB, _ := r.ReadTestByID(ctx, test.ID)

	if test.Name != "" {
		query := `UPDATE testing_info.test SET name = $1 WHERE id = $2`
		_, err := tx.ExecContext(ctx, query, test.Name, test.ID)
	...
	var catsInDB, cats []int

	query := `SELECT category_id FROM testing_info.test_category WHERE task_id = $1`

	if err := tx.GetContext(ctx, catsInDB, query, test.ID); err != nil {
	...
	catsToAdd := difference(cats, catsInDB)
	catsToDelete := difference(catsInDB, cats)

	query = `DELETE FROM testing_info.test_category WHERE id IN (?)`

	q, args, err := sqlx.In(query, catsToDelete)
	if err != nil {
	...
	query = `INSERT INTO testing_info.test_category(category_id, test_id) VALUES (:category_id`

	// catsToKeep := make([]int, 0)
	// catsToDelete := make([]int, 0)

	// for _, catInDB := range testInDB.Categories {
	// 	isCategoryKept := false
	// 	for _, cat := range test.Categories {
	// 		isCategoryKept = isCategoryKept || (cat.ID == catInDB.ID)
	// 		if isCategoryKept {
	// 			break
	// 		}
	// 	}
	// 	if isCategoryKept {
	// 		catsToKeep = append(catsToKeep, catInDB.ID)
	// 		continue
	// 	} else {
	// 		catsToDelete = append(catsToDelete, catInDB.ID)
	// 		continue
	// 	}
...
// Read category
func (r *Repository) getCategoryDependencies(ctx context.Context, cat *domain.Category) error {
...
func (r *Repository) GetCategoriesByTestId(ctx context.Context, testID int) ([]domain.Category, error) {
	const query = `SELECT id, name FROM testing_info.category c 
	JOIN testing_info.test_category tc on c.id = tc.category_id
	WHERE tc.test_id = $1`
	var categories []domain.Category

	if err := r.db.SelectContext(ctx, &categories, query, testID); err != nil {
...
func (r *Repository) GetSchemasOfDBMS(dbms string) ([]string, error) {
